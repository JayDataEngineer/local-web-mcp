...
class Database:
    ...
    async def clean(self) -> int:
        ...
        if self._pool is None:
            logger.warning("Database not available, nothing to clean")
            return 0
        async with self._pool.acquire() as conn:
    ...
    async def cleanup_old_blacklisted(self, days_old: int = 7) -> int:
        """
        Remove blacklisted domains that haven't been updated in X days

        Args:
            days_old: Remove domains where updated_at < NOW() - INTERVAL 'X days'
        ...
        async with self._pool.acquire() as conn:
            ...
            count_int = int(count)
            logger.info(f"Cleaned up {count_int} blacklisted domains older than {days_old} days")
            return count_int


# Async singleton factory
from ..utils import create_async_singleton_factory
get_db = create_async_singleton_factory(Database, "get_db", init_method="init")
